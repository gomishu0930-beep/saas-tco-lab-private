...
const drafts: Readonly<Record<PilotPage["id"], ArticleDraft>> = {
  ...
  P05: {
    ...
    reviewVoices: ["analyst", "editor", "skeptical_buyer"],
    sections: [
      { title: "判断の要点", body: "Mangools Agencyの年次checkout総額と、追加できるseat数、Site analysisの24時間上限を確認する。管理者数や月間監査ページ数へ読み替えず、公式表記の単位で判断する。" },
      { title: "組織の条件", body: "必要な利用人数、Site analysisの実行頻度、移行支援の要否を先に整理する。追加seat数は総seat数や管理者数ではないため、権限要件は購入前に別途確認する。" },
      { title: "料金と利用上限", body: "Agencyで追加可能なseat数{{contract:enterprise.agency_extra_seats_available.value}}、年次checkout総額{{contract:enterprise.agency_annual_checkout_total.value}}、Site analysis上限{{contract:enterprise.site_analysis_requests_per_24h.value}}を、それぞれの公式単位のまま示す。" },
      { title: "12か月で考える", body: "年次checkout総額を一次値として扱い、移行支援費{{contract:enterprise.migration_support_price.value}}は確認できるまで加えない。移行支援の記載が見つからないことを0円の根拠にはしない。" },
      { title: "契約前の注意", body: "追加seatの対象、権限の範囲、Site analysisの更新条件、移行支援の提供有無を確認する。異なる機能上限を同じ『監査』指標として比較しない。" },
      { title: "出典と更新", body: sourceBlock },
    ],
  },
  P06: {
  ...
  P12: {
    ...
    sections: [
      ...
      { title: "更新のタイミング", body: "全field共通の確認間隔は設けず、値ごとの次回確認日を使う。価格や条件が変わった場合は期限前でも再確認し、期限を過ぎた数値は再確認まで計算から外す。" },
      { title: "出典と更新", body: sourceBlock },
    ],
  },
};
