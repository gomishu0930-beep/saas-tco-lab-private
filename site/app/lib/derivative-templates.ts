...
const verifiedDerivativeSpecs: Readonly<Partial<Record<PilotPage["id"], VerifiedDerivativeSpec>>> = {
  ...
  P09: {
    title: "実移行で確認したHuman作業費は2,006.67 JPY",
    summary: "2026年8月13日の実移行では、移行作業1,177秒、教育27秒でした。Human設定の時間単価6,000 JPY/時を適用した作業費は2,006.67 JPYです。重複契約は0か月、公式移行支援費は未確認のため、移行費用全体の確定額ではありません。",
    sections: [
      { title: "実測した作業時間", body: "移行作業は1,177秒（0.32694444時間）、教育は27秒（0.0075時間）でした。開始・停止後に秒数を個別確認した1回の実移行記録です。" },
      { title: "Human作業費", body: "移行作業と教育の合計0.33444444時間に、Human設定の時間単価6,000 JPY/時を掛けると2,006.67 JPYです。公式料金ではなく、今回の機会費用scenarioとして分けています。" },
      { title: "重複契約", body: "今回の重複契約期間は0か月でした。この実測を、別サービスや別の移行条件でも必ず0か月になるという一般則にはしません。" },
      { title: "未確認の公式支援費", body: "公式価格ページで移行支援料金を確認できていません。記載が見つからないことを無料の根拠にせず、2,006.67 JPYへ0円として加えていません。" },
      { title: "この数字を使える範囲", body: "対象は今回完了したSaaS TCO Labの実移行1回です。移行データ量、権限、停止時間、担当者が異なる場合は作業時間を再計測します。" },
      { title: "結論", body: "確認できたHuman作業費は2,006.67 JPYです。公式支援費などの未確認項目が残るため、移行費用全体は確定していません。" },
    ],
    xPoints: [
      "実移行の作業は1,177秒、教育は27秒。合計0.33444444時間をHuman確認しました。(2/8)",
      "Human設定の時間単価は6,000 JPY/時。確認済み作業費は2,006.67 JPYです。(3/8)",
      "重複契約は0か月でした。ただし別の移行でも0か月になるとは一般化しません。(4/8)",
      "公式移行支援費は未確認。記載がないことを無料の根拠にせず、0円として合計しません。(5/8)",
      "この値はSaaS TCO Labで完了した実移行1回の記録です。データ量や担当者が変われば再計測します。(6/8)",
      "確認済みHuman作業費は2,006.67 JPY。未確認費用が残るため、移行費用全体は未確定です。(7/8)",
    ],
    articleUrl: "https://saastcolab.jp/pilot/migration-cost",
