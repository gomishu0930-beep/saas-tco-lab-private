...
公開Affiliate Programは5社すべてで確認済みです。Mangoolsはaffiliate accessと紹介素材の発行まで到達し、HubSpotは2026-07-26にImpact申請を送信して審査待ち、Semrushは同日に契約同意を完了しましたがImpact login待ちで申請未送信です。ただし、申請中や単なるaccount発行を「実利用可能な提携」と数えず、公開料金ページを継続取得し、最小fieldを保存し、比較表・12か月TCO・価格履歴として商用公開する一連の権利を明示確認できた会社も0社です。したがって統合gateは現時点で **0/3、STOP** です。
...
|---|---|---|---|---|---|---|---|---|
|Semrush|**回答で禁止**: website dataの自動収集は不可。公式APIは別規約|**回答で禁止**: website pricing/featuresの継続保存は不可|**条件付き可**: 正確・最新の独立記事。系統的field転載は不可|**未回答**|**回答で禁止**: website dataの履歴DB化は不可|**条件付き可**: publication ruleとAffiliate Termsに従う|**契約同意済み・login待ち・未申請**|data sourceはSTOP。手動検証記事だけ候補|
...
|Affiliate|契約同意済み / 未申請|2026-07-26にHuman承認で契約同意。既存Impact loginのcredential store解除待ち|
...
|---|---|---|---|---|---|---:|
|Semrush|確認済み|Impact契約同意済み・login待ち・申請未送信|未取得|未取得|自動取得・継続保存・履歴は明示不可|0|
