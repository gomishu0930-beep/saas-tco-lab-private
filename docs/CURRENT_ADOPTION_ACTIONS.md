...
2026-08-19、servers 40語をKWFinder正規画面のJapan指定で再exportし、凍結slate完全一致を確認しました。
known 14、no_data 26、rejected 0、known合計28,220/月は前回と一致しています。上位2語比率は
71.934798%、上位5語比率は97.271439%で、固定規則の高集中判定です。したがって28,220/月を実質
addressable需要と同一視せず、後者は`unknown_headword_concentrated`とします。raw CSVとquery別volumeは
repositoryへ保存していません。また、公開対象8 vendorの指名複合語30語を`brand_servers`として凍結し、
同日に30/30をHuman exportしました。known 2、no_data 28、rejected 0、known合計490/月、no_data率
93.333333%で、証拠不足のため需要不足を確定せずgeneric serversへ合算しません。AIサイト構築サービスの普及による比較需要縮小はC004で、同一slateの
年次再測定が前年同月比-20%以上になった時に検知します。
...
2026-08-11、P09/P11の自データ待ちを手書きJSONなしで解消できるよう、既存`/operator`へlocal-onlyの
実測入力cardを追加した。P09は完了した実移行1回の重複契約月数・移行作業時間・教育時間とHuman設定の
時間単価を分離して候補化する。P11は導入前後の完全な暦月を1か月ずつ入力し、月途中の外挿を行わず、
月間時間差を基準月−比較月で決定論的に算出する。負値と0は隠さず警告表示する。候補はHuman確認前に
...
- 2026-08-19、直前に列挙した8記事を対象とする`index_go: GO`を受領した。
  ...
  `CHECK-ALL: PASS`（Python 858件、site local 83件、startup 1件、production 18件、secret scan）後、
  Sites version 33へproduction反映した。外部read-backでは9本のserver記事がすべてHTTP 200・
  `index, follow`・self-canonical、sitemap 20 URL、robotsの9本Allow、homeの公開記事card 20本を確認した。
  sponsored CTAはSVR01の既存6リンクだけで、新8記事は0件、P11は`noindex, follow`・CTA 0件を維持する。
- M5 dashboardは8月末4条件を別枠で機械集計する。比較READYは承認済み数値fieldに加えて、対応する
  ...
  完全月read-backまで確定しない。
- 2026-08-19のread-only再確認で、GSC sitemapは成功・最終読み込み2026-08-19・検出20となり、
  productionの公開20記事と一致した。index登録7・未登録6、aggregate表示13・clicks 0であり、検出20を
  index完了へ読み替えない。GA4 Realtimeは0、直近7日は`page_view` 7・`qualified_session` 2・
  `outbound_click` 0である。内部訪問分離不能のため月次KPIへ転記せず、8月31日の完全月read-backを待つ。
  外部read-backでは20記事の`index, follow`・self-canonical、P11の`noindex, follow`・CTA無効、
  P記事11本のMangools CTA、SVR01だけのserver CTA 6リンク、開示先行とlink属性を再確認した。

## 2026-08-20 balanced 14-day growth sprint

- Human Approverは`growth_sprint: GO balanced_14d`、`svr01_cta_experiment: GO primary_1 alternative_1`、
  `server_internal_funnel: GO SVR02-SVR09 -> SVR01`、`repository_scope_prepare: GO`を承認した。
- SVR01は既存のHuman確認順を変えず、記事対象のXServerビジネスをprimary枠、確認済み別候補の
  ConoHa WINGをalternative枠とする。残る4件の承認programは台帳・runtime gateを維持するが、
  この14日実験のSVR01画面には表示しない。価格だけの最安・おすすめ判定ではない。
- SVR02–SVR09はaffiliate CTA無効を維持し、各記事の意図に合わせたSVR01への内部比較linkを1本追加する。
- `calculator_result_view`、`cta_view`、`cta_eligible_session`、`server_internal_funnel`はanalytics consent後だけ
  safe parameterで計測する候補とする。query、tracking ID、Affiliate URL、PIIは送らない。
- `ga4_internal_filter: HOLD`、`hub_index: HOLD`、`external_posts: DRAFT_ONLY`を維持する。
  今回のscopeはpush、deploy、外部投稿、本番設定変更、新規記事、SVR02–SVR09のaffiliate CTAを含まない。
- 詳細な作業・判定条件は`outputs/GROWTH_SPRINT_14D_2026-08-20.md`、投稿下書きは
  `outputs/EXTERNAL_DRAFTS_SVR01_2026-08-20.md`を参照する。両者はpush対象外の作業出力である。
