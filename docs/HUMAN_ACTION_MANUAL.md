...
|Search Console / GA4|`saastcolab.jp`のGSC domain property所有確認・専用GA4 stream連携済み。GA4は同意前・拒否後0通信、同意後Realtime `page_view`受信済み。公開sitemapは10記事、GSC検出は9記事で次回crawl待ち。internal traffic filterは安全のため`test`を維持|
