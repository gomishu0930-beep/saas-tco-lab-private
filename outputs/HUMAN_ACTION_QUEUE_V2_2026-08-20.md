...
|2|Cell B article / vendor|SVR04とXServerビジネスを確定|`cell_b: GO SVR04 xserver-business`を2026-08-22受領|launch state / runtime article allowlist|0|owned-site単独CTAをrelease候補へ登録|gate不一致時はaffiliate slot無効|done|
...
|5|channel / deep link|A8.netのnote・X・partner配信とsub ID可否|一般公開案内はsite/note/SNS掲載例を確認。program固有条件はA8.net session失効で未確認|A8.net program詳細へ再login後にsafe-summary|20|確認済みchannelだけcampaign運用|owned-site以外はdraft、deep link/sub ID無効|2026-08-24|
...
|7|GA4 internal filter|現回線definition rule作成済み。別回線の誤除外確認待ち|`traffic_type=internal`、データフィルタは`テスト`をread-back済み|GA4 DebugView / Realtime|20|別回線検証後にだけactiveを再判断|test維持、KPIを収益証拠にしない|2026-08-24|
...
```text
ga4_internal_filter: TEST / definition rules 1
...
cell_b: GO SVR04 xserver-business / owned_site_only
```
