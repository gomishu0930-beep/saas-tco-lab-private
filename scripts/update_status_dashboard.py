...
def _external_action_queue(
    ...
    *,
    ...
    servers_demand_summary_dir = (
        root / "artifacts" / "editorial-inputs" / "demand-safe-summaries"
    )
    local_demand_summary_dir = root / "outputs" / "demand-safe-summaries"
    server_demand_summaries = sorted(
        (
            *servers_demand_summary_dir.glob("servers-*.json"),
            *local_demand_summary_dir.glob("servers-*.json"),
        ),
        key=lambda path: path.name,
        reverse=True,
    )
    servers_concentration_pending = True
    if server_demand_summaries:
        servers_demand_summary_path = server_demand_summaries[0]
        servers_demand_summary = json.loads(
            servers_demand_summary_path.read_text(encoding="utf-8")
        )
        servers_concentration_pending = (
            servers_demand_summary.get("top_2_known_volume_share") is None
            or servers_demand_summary.get("top_5_known_volume_share") is None
        )
    if servers_concentration_pending:
        actions.append({
            "label": "servers 40語を同一条件で再exportし、headword集中度を確定",
            "status": "human_export_required",
            "token": "mangools_servers_concentration_csv: done",
        })
    brand_server_summaries = (
        *servers_demand_summary_dir.glob("brand_servers-*.json"),
        *local_demand_summary_dir.glob("brand_servers-*.json"),
    )
    if not any(brand_server_summaries):
        actions.append({
            "label": "brand_servers 30語をHuman export",
            "status": "human_export_required",
            "token": "mangools_brand_servers_csv: done",
        })
    for research_id in (
...
def _regenerate(
    ...
    *,
    ...
    server_template_source = (
        root / "site" / "app" / "components" / "PilotArticle.tsx"
    ).read_text(encoding="utf-8")
    displayed_server_cta_ids = sorted(set(re.findall(
        r'data-server-affiliate-cta-placeholder="([a-z0-9-]+)"',
        server_template_source,
    )).intersection(enabled_server_cta))
    ...
    data["serverLaunch"] = {
        ...
        "ctaDisplayedPartners": len(displayed_server_cta_ids),
    ...
    if "GSC sitemapは成功・最終読み込み2026-08-19・検出20" in adoption:
        dynamic_risks.append({
            "riskId": "gsc-processing",
            "level": "monitor",
            "label": "GSC sitemapは20/20検出済み。index登録7・未登録6・表示13・clicks 0の再処理を監視",
        })
        dynamic_risks.append({
            "riskId": "ga4-traffic-evidence",
            "level": "monitor",
            "label": "GA4直近7日はpage_view 7・qualified_session 2・outbound_click 0。内部訪問分離不能のため収益証拠へ未算入",
        })
    elif "sitemapは成功・12ページ検出" in adoption:
        dynamic_risks.append({
            "riskId": "gsc-processing",
            "level": "monitor",
    ...
    elif "Sites version 31へdeploy" in adoption and "未承認・管理routeのfail-closedを外部read-back済み" in adoption:
        dynamic_risks.append({
            "riskId": "index-discovery-path",
            "level": "monitor",
    ...
    if published_server_count:
        ...
        dynamic_risks.append({
            ...
            "level": "warn",
            "label": (
                f"server記事{published_server_count}本は承認・index対象。SVR01は14日実験で"
                f"{len(displayed_server_cta_ids)}件表示・runtime GO {len(enabled_server_cta)}件・"
